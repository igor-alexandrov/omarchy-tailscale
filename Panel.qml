...
Panel {
  ...
  // The bar owns the "omarchy.tailscale" target and its handler does not drive
  // this instance, so the panel cannot otherwise be opened by command. This
  // handler uses a target of its own and calls straight into the panel, which
  // makes the widget scriptable and bindable to a hotkey.
  IpcHandler {
    target: "igor-alexandrov.tailscale"
    enabled: true
    function open(): void { root.open() }
    function close(): void { root.close() }
    function toggle(): void { root.toggle() }
    function isOpen(): string { return root.opened ? "open" : "closed" }
    function adminConsole(): void { root.openAdminConsole() }
  }
  ...
  readonly property string adminConsoleUrl: "https://login.tailscale.com/admin/machines"
  readonly property bool showSelfDevice: tailscale.installed && tailscale.selfName !== ""
  ...
  // Opens the tailnet admin console in the default browser. The panel closes
  // first so the browser does not come up behind a floating layer surface.
  function openAdminConsole() {
    close()
    Quickshell.execDetached(["xdg-open", root.adminConsoleUrl])
  }
  ...
  function moveCursor(dx, dy) {
    ...
    if (dy !== 0) {
      ...
      } else if (focusSection === "admin") {
        // Always the last row, so only upward movement has anywhere to go.
        if (dy < 0) {
          if (showPeers) { focusSection = "peers"; peerIndex = tailscale.peers.length - 1 }
          else if (showExitNodes) { focusSection = "exitNodes"; exitNodeIndex = exitNodes.length - 1 }
          else if (tailscale.accounts.length > 1) { focusSection = "accounts"; accountIndex = tailscale.accounts.length - 1 }
  ...
  // This machine's own identity on the tailnet. Informational, so it carries no
  // cursor state; the copy button is the only interactive part.
  component SelfDeviceRow: Item {
    id: selfRow
    readonly property string subtitleText: {
      var parts = []
      if (tailscale.selfIp !== "") parts.push(tailscale.selfIp)
      if (tailscale.selfDnsName !== "") parts.push(tailscale.selfDnsName)
      return parts.join(" \u00b7 ")
    }

    implicitHeight: selfInner.implicitHeight + Style.spacing.xl
    ...
    RowLayout {
      ...
      Text {
        textFormat: Text.PlainText
        text: tailscale.osIcon(tailscale.selfOs)
        color: root.foreground
        font.family: root.fontFamily
      ...
      PanelActionButton {
        iconText: "\udb80\udd8f"
        tooltipText: "Copy this device's IP"
        foreground: root.foreground
        fontFamily: root.fontFamily
        enabled: tailscale.selfIp !== ""
        Layout.alignment: Qt.AlignVCenter
        onClicked: tailscale.copyToClipboard(tailscale.selfIp, tailscale.selfName + " IP")
      }
    }
  }

  // Always-visible shortcut to the tailnet admin console. The CONNECTIONS rows
  // above only render with more than one login, so this is the dependable way
  // to reach the account from the panel.
  component AdminConsoleRow: CursorSurface {
    id: adminRow
    readonly property string accountText: {
      if (tailscale.selectedAccountLabel !== "") return String(tailscale.selectedAccountLabel)
      // Fall back to the tailnet from the machine's MagicDNS name
      // (host.tailnet.ts.net), which is present whenever the CLI reports status.
      var dns = String(tailscale.selfDnsName || "")
      var dot = dns.indexOf(".")
      if (dot > -1 && dot < dns.length - 1) return dns.slice(dot + 1)
      return "login.tailscale.com"
    }

    hasCursor: root.cursorActive && root.focusSection === "admin"
    foreground: root.foreground
    fill: root.hoverFill
    currentFill: root.selectedFill

    implicitHeight: adminInner.implicitHeight + Style.spacing.xl
  ...
  component PeerRow: CursorSurface {
    ...
    RowLayout {
      ...
      PanelActionButton {
        id: sendButton
        visible: peerRow.peerOnline && tailscale.canSendFiles(peerRow.peer)
        iconText: "󰒊"
